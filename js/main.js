...
(function(){
  ...
  function renderProjects(data){
    ...
    var projectPhotos = ["project-refinery.jpg","project-pipeline.jpg","project-pumpjack.jpg","project-night.jpg","project-engineers.jpg"];
    var photoPrefix = (isInPagesDir() ? "../" : "") + "assets/images/photos/";

    wrap.innerHTML = showItems.map(function(p, i){
      var photo = projectPhotos[i % projectPhotos.length];
      return '<article class="project-card reveal">' +
        '<div class="thumb"><img src="'+ photoPrefix + photo +'" alt="'+ p.title +'" loading="lazy"></div>' +
  ...
  function renderCompanyLocation(data){
    if(!window.COMPANY_INFO) return;
    var text = window.COMPANY_INFO.location[data.lang];
    document.querySelectorAll("[data-company-location]").forEach(function(el){ el.textContent = text; });
  }
